...
using RTSEngine.AI.Event;

namespace RTSEngine.AI.Attack
{

    /// <summary>
    /// Responsible for picking a target faction and launching attacks for a AI faction
    /// </summary>

    public class AIAttackManager : AIComponentBase, IAIAttackManager
    {
        #region Attributes
        [Header("Picking Target")]
        [SerializeField, Tooltip("Can the AI faction attack other factions? Disable to deactivate this component's behavior.")]
        ...
        [SerializeField, Tooltip("What's the AI faction's strategy when picking a target faction?")]
        private AIAttackTargetPickerType targetFactionType = AIAttackTargetPickerType.random;
        ...
        [SerializeField, Tooltip("How often does the AI faction consider whether to launch an attack or not?")]
        private FloatRange launchAttackReloadRange = new FloatRange(10.0f, 15.0f);
        private TimeModifiedTimer launchAttackTimer;

        // Is the AI faction currently engaging in an attack towards its current target faction?
        ...
        [SerializeField, Tooltip("How often does the AI faction pick the next target unit/building to attack while engaging in an attack towards enemy faction?"),]
        ...
        [SerializeField, Tooltip("Define the buildings/units that the AI faction will prioritize targeting during an active attack.")]
        ...
        public enum AIAttackFactionEntityPreference { random, units, buildings }

        [SerializeField, Tooltip("AI faction preferences when picking the next faction entity target using this component.")]
        private AIAttackFactionEntityPreference targetPreference = AIAttackFactionEntityPreference.random;
        ...
        // AI Components
        protected IAIDefenseManager AIDefenseMgr { private set; get; }
        protected IAIEventPublisher AIEvent { private set; get; } 
        ...
        protected override void OnPreInit()
        {
            ...
            this.AIDefenseMgr = AIMgr.GetAIComponent<IAIDefenseManager>();
            this.AIEvent = AIMgr.GetAIComponent<IAIEventPublisher>();
        ...
        {
            // When the AI faction is eliminated, cancel the current attack
        ...
        public void SetTargetFaction()
        {
            ...
            {
                case AIAttackTargetPickerType.random:
                    targetFactionSlot = activeFactions[UnityEngine.Random.Range(0, activeFactions.Length)];
                    break;

                case AIAttackTargetPickerType.mostAttackResources:
                    targetFactionSlot = activeFactions[0];
                    break;

                case AIAttackTargetPickerType.leastAttackResources:
        ...
        {
            // Can not launch a new attack if the AI faction is already in a defensive mode
            if (AIDefenseMgr.IsDefending)
                return;
            ...
            if (launchAttackTimer.ModifiedDecrease())
            {
                ...
                // Does the AI faction has enough attacking power to launch attack?
        ...
        /// <summary>
        /// Launches an attack on the AI faction's current target faction.
        /// </summary>
        ...
        /// <param name="potentialTarget"> The entity that is targeted by the agent </param>
        ...
        private void OnActiveAttackUpdate()
        {
            ...
            bool randPickBuildingsPrimary = targetPreference == AIAttackFactionEntityPreference.random
                && UnityEngine.Random.value > 0.5f;

            if (targetPreference == AIAttackFactionEntityPreference.buildings
        ...
        /// <summary>
        /// Stops the AI faction from attacking and resets its targets.
        /// </summary>
        public void CancelAttack()
        {
            if(IsAttacking)
                AIEvent.RaiseAttackCancelled(this);
        ...
        /// <summary>
        /// Handle the engagement of the target if the game is not in peace, the target is valid / alive, and the unit is not already attacking something else
        /// </summary>
        private bool EngageCurrentTargetFactionEntity()
        {
            ...
            AIEvent.RaiseAttackEngageOrder(this, new AIAttackEngageEventArgs(currentTargetEntity, lastAttackPos));
...
#if UNITY_EDITOR
        [System.Serializable]
        private struct AIAttackLogData 
        {
        ...
        private AIAttackLogData attackLogs = new AIAttackLogData();
        ...
        {
            attackLogs = new AIAttackLogData
            {
